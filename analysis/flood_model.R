...
data_grouppatch <- data_filter %>%
  dplyr::select(
    timestamp, y = !!control$flux, dplyr::matches("_wet$|_dry$")
  ) %>%
  tidyr::pivot_longer(
    c(-timestamp, -y), names_to = c("c", "q"), values_to = "p", names_sep = "_"
  ) %>%
  tidyr::pivot_wider(names_from = c, values_from = p) %>%
  dplyr::mutate(q = as.integer(factor(q, c("wet", "dry"))))
# wet = 1, dry = 2
...
fit1a <- rjags::jags.model(
  textConnection(mod1a), data, n.chains = 3, n.adapt = 1000, inits = inits1a
)
...
# Model 1B: Vague priors

set.seed(seeds$init)
inits1b <- list(
  list(
    beta1 = rep(rnorm(1, 0, 100), n.j),
    beta2 = rep(rnorm(1, 0, 100), n.j), 
    beta3 = rep(rnorm(1, 0, 100), n.j),
    sigma = runif(1, 0, 100)
  ),
  list(
    beta1 = rep(rnorm(1, 0, 100), n.j),
...
fit1b <- rjags::jags.model(
  textConnection(mod1b), data, n.chains = 3, n.adapt = 1000, inits = inits1b
)
...
fit2a <- rjags::jags.model(
  textConnection(mod2a), data, n.chains = 3, n.adapt = 1000, inits = inits2
)

update(fit2a, n.iter = 10000)

samps2a <- rjags::coda.samples(fit2a, params2, n.iter = 10000)
...
S <- eivtools::get_bugs_wishart_scalemat(
  c(priors$sigma2[1], priors$sigma2[2], priors$sigma2[3])
)
#S$bugs.scalemat

mod2b <- as.character(glue::glue("
data {{

  # scale matrix for Wishart prior on precision matrix
  S[1, 1] <- {S$bugs.scalemat[1, 1]}
  S[1, 2] <- 0
  S[1, 3] <- 0
  S[2, 1] <- 0
  S[2, 2] <- {S$bugs.scalemat[2, 2]}
  S[2, 3] <- 0
  S[3, 1] <- 0
  S[3, 2] <- 0
  S[3, 3] <- {S$bugs.scalemat[3, 3]}
...
model {{
  ...
  # fixed effects
  mu.beta1 ~ dnorm({priors$mu[1]}, {priors$tau[1]})
  mu.beta2 ~ dnorm({priors$mu[2]}, {priors$tau[2]})
  mu.beta3 ~ dnorm({priors$mu[3]}, {priors$tau[3]})
...
fit2b <- rjags::jags.model(
  textConnection(mod2b), data, n.chains = 3, n.adapt = 1000, inits = inits2
)

update(fit2b, n.iter = 20000)

samps2b <- rjags::coda.samples(fit2b, params2, n.iter = 20000)
